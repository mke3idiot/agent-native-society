...
This registry connects the organizational functions to concrete loops. A loop moves from proposal to autonomous execution only after its inputs, effects, and tests are explicit.
...
1. Architecture/specification consistency check for ARCG contracts and tests.
2. A research synthesis loop that transforms a reviewed source-change PR into a dated draft with facts and hypotheses separated.
3. A monthly CEO briefing that reports only decisions, risks, and opportunities—never raw activity volume.
